#!/usr/bin/env node
/**
 * SNOMED CT Code Validator - Snowstorm Training Instance
 * Uses working Snowstorm training API
 *
 * API: https://snowstorm-training.snomedtools.org/snowstorm/snomed-ct/
 */

const https = require('https');

const SNOWSTORM_BASE = 'https://snowstorm-training.snomedtools.org/snowstorm/snomed-ct/browser/MAIN/concepts';

/**
 * Lookup a SNOMED CT code using Snowstorm browser API
    ...
    https.get(url, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode === 200 && result.conceptId) {
            resolve({
              code: code,
              valid: true,
              display: result.pt?.term || result.fsn?.term || 'Unknown',
              fsn: result.fsn?.term,
            ...
            resolve({
              code: code,
              valid: false,
              error: result.message || 'Code not found',
              fullResponse: result
            });
          }
        } catch (error) {
